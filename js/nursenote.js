document.addEventListener("DOMContentLoaded", function () {
  ...
  const nurseNotesTable = document
    .getElementById("nurse-notes-table")
    .getElementsByTagName("tbody")[0];
  const nurseNoteSearchOutput = document.getElementById(
    "nurse-note-search-output"
  );
  const updateNurseNoteIDSelect = document.getElementById(
    "update-nurse-note-id"
  );
  ...
  function loadNurseNotes() {
    fetch("https://nursenotes.somee.com/apiNurseNote")
      ...
      .then((data) => {
        ...
        updateNurseNoteIDSelect.innerHTML =
  ...
  nurseNoteCreateForm.addEventListener("submit", (e) => {
    ...
    const incomE_ID = parseInt(
      document.getElementById("nurse-income-id").value
    );
    const patienT_ID = parseInt(
      document.getElementById("nurse-patient-id").value
    );
    const reasoncons = document.getElementById("nurse-reason-consult").value;
    const diaG_ID = parseInt(
      document.getElementById("nurse-diagnosis-id").value
    );
    const speC_ID = parseInt(
      document.getElementById("nurse-specialty-id").value
    );
    ...
    fetch("https://nursenotes.somee.com/apiNurseNote", {
      ...
      .catch((error) =>
        console.error("Error en la creación de la nota de enfermería:", error)
      );
  });
  ...
  updateNurseNoteIDSelect.addEventListener("change", () => {
    ...
    fetch(`https://nursenotes.somee.com/apiNurseNote/${selectedNoteId}`)
      ...
      .catch((error) =>
        console.error(
          "Error al cargar los datos de la nota de enfermería:",
          error
        )
      );
  });
  ...
  updateNurseNoteBtn.addEventListener("click", (e) => {
    ...
    const reasoncons = document.getElementById(
      "update-nurse-reason-consult"
    ).value;
    const diaG_ID = parseInt(
      document.getElementById("update-nurse-diagnosis-id").value
    );
    ...
    const usR_ID = parseInt(
      document.getElementById("update-nurse-user-id").value
    );
    const stafF_ID = parseInt(
      document.getElementById("update-nurse-staff-id").value
    );
    ...
    fetch(`https://nursenotes.somee.com/apiNurseNote/${notE_ID}`, {
      ...
      .catch((error) =>
        console.error(
          "Error en la actualización de la nota de enfermería:",
          error
        )
      );
  });
